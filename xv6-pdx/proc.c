...
procdumpP2P3P4(struct proc *p, char *state_string)
{
  ...
  cprintf("\n%d\t%s\t\t%d\t%d\t%d\t%d.%d%d%d\t%d.%d%d%d\t%s\t%d\t",p->pid, p->name, p->uid, p->gid, ppid, secs, hundreds, tens, ones, cpu_s, cpu_hundreds, cpu_tens, cpu_ones, state_string, p->sz);
...
procdumpP1(struct proc *p, char *state_string)
{
	...
	cprintf("%d\t%s\t\t%d.%s%d\t%s\t%d\t", p->pid,p->name,elapsed_s,zeros,elapsed_ms,state_string,p->sz);
